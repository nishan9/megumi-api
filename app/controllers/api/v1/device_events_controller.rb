    ...
    class DeviceEventsController < ApplicationController
      ...
      def update
        ...
        if device_event
          if device_event.notification_sent?
            render json: { error: 'Notification has already been sent for this event' },
                   status: :unprocessable_entity
          elsif device_event.update(notification_sent: true)
            render json: device_event, status: :ok
          else
      ...
      def destroy
        ...
        if device_event
          ...
          elsif device_event.update(is_deleted: true)
